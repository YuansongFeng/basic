...
def get_subsequent_mask(seq_output):
    ...
    # B x N x N
    subsequent_mask = subsequent_mask.unsqueeze(0).repeat(seq_output.size(0), 1, 1).eq(1)
...
class Transformer(nn.Module):
    ...
    def forward(self, inputs, outputs):
        ...
        # B x N_out x N_out
        dec_self_att_mask = get_att_padding_mask(seq_q=outputs, seq_k=outputs, pad_label=self.pad_label) | get_subsequent_mask(outputs)
